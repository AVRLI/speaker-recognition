 ...
 * @{
 */
void pre_emphasis(const float32_t* input, float32_t* output, uint32_t len) {

    // The first element of the output is the same as the input
    output[0] = input[0];

    // Apply the pre-emphasis filter to the rest of the elements
    for (int i = 1; i < len; ++i) {
 ...
 */
void buf_to_float(int32_t *pInSignal, float32_t *pOutSignal, uint32_t len)
{
 ...
 * @param      len          signal length
 */
void buf_to_float_normed(int32_t *pInSignal, float32_t *pOutSignal, uint32_t len)
{
  for (uint32_t i = 0; i < len; i++)
  {
